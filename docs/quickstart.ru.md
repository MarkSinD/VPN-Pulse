...
- цикл мониторинга, хранилище и модель чтения на SQLite, API и Mini App, CLI администратора и
  установщик — вся локальная цепочка покрыта одним сквозным тестом;
- демо на вымышленных данных (`./install.sh demo` или `python -m vpnpulse.dev`), где состояние
  меняется за минуты; настоящая установка показывает «нет данных», пока не появятся сборщики и пробники;
- проверенные контракты (OpenAPI 3.1, JSON Schema, словари RU/EN) и интерактивные прототипы всех
  экранов, включая ошибки и первый запуск.

## Требования

- Python 3.12 или новее (на сервере — Ubuntu 24.04);
- браузер для Mini App и прототипов.
