...
if [ "${1:-}" = "--remove" ]; then
  ...
  systemctl disable --now vpn-pulse-probe-netns.service 2>/dev/null || true
  rm -f /etc/systemd/system/vpn-pulse-probe-abroad.service /etc/systemd/system/vpn-pulse-probe-abroad.timer /etc/systemd/system/vpn-pulse-probe-netns.service /usr/local/bin/vpn-pulse-probe-abroad
...
install -m 0644 "$HERE/vpn-pulse-probe-netns.service" /etc/systemd/system/
systemctl daemon-reload
systemctl start vpn-pulse-probe-netns.service
