...
(function () {
  ...
  async function flushAnalytics(keepalive) {
    ...
    let ok = false;
    try { await api.analytics(batch, keepalive); analytics.queue.splice(0, batch.length); analytics.sent += batch.length; ok = true; }
    catch (e) {
  ...
  async function ensureSession() {
    ...
    if (tg && tg.initData) { await api.session(tg.initData); const me = await api.me(); S.role = me.role; S.sessionRole = S.role; }
    else {
  ...
  ensureSession().catch(() => {}).finally(() => track('app_opened', { role: S.role, language: S.lang, theme: S.theme, viewport_bucket: window.innerWidth < 480 ? 'phone' : window.innerWidth < 1024 ? 'tablet' : 'desktop' }));
