...
(function () {
  ...
  // a rejected batch is dropped, an unreachable API is retried on the next flush.
  const events = window.__vpnPulseEvents = [];
  const analytics = { queue: [], timer: null, session: uuid(), inflight: false, sent: 0 };
  function uuid() {
  ...
  async function flushAnalytics() {
    ...
    try { await api.analytics(batch); analytics.queue.splice(0, batch.length); analytics.sent += batch.length; }
    catch (e) { if (e && e.status === 400) analytics.queue.splice(0, batch.length); }
    finally { analytics.inflight = false; }
    if (analytics.queue.length && S.sessionRole) scheduleFlush(0);
  }
